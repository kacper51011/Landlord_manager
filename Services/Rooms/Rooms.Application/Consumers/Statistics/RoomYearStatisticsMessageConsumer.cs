    ...
    public class RoomsYearStatisticsMessageConsumer : IConsumer<StatisticYearMessage>
    {
        ...
        public async Task Consume(ConsumeContext<StatisticYearMessage> context)
        {
            try
            {
                ...
                RoomsStatistics apartmentStatistic = RoomsStatistics.CreateAsYearStatisticsInformations(context.Message.Year);
                await _roomsStatisticsRepository.CreateOrUpdateRoomStatistics(apartmentStatistic);
                _logger.LogInformation($"Year apartment statistic created with Id {apartmentStatistic.RoomsStatisticsId}");
                return;
            }
